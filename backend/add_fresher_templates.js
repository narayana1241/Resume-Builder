...
// Canvas Renderer for Preview PNGs
function generatePreviewPNG(folderName, tplData) {
  ...
  // Template specific colors & styling
  let primaryColor = "#1e1b4b";
  let accentColor = "#06b6d4";
  let isDarkSidebar = false;

  if (folderName === "Template-016") { primaryColor = "#1e1b4b"; accentColor = "#06b6d4"; }
  else if (folderName === "Template-017") { primaryColor = "#0f172a"; accentColor = "#10b981"; isDarkSidebar = true; }
  else if (folderName === "Template-018") { primaryColor = "#1e293b"; accentColor = "#10b981"; }
  else if (folderName === "Template-019") { primaryColor = "#701a75"; accentColor = "#d97706"; }
  else if (folderName === "Template-020") { primaryColor = "#4c1d95"; accentColor = "#7c3aed"; }
  else if (folderName === "Template-021") { primaryColor = "#0c4a6e"; accentColor = "#0d9488"; }
  else if (folderName === "Template-022") { primaryColor = "#1e293b"; accentColor = "#f43f5e"; }
  else if (folderName === "Template-023") { primaryColor = "#9f1239"; accentColor = "#be123c"; }
  else if (folderName === "Template-024") { primaryColor = "#18181b"; accentColor = "#15803d"; }
  else if (folderName === "Template-025") { primaryColor = "#9a3412"; accentColor = "#ea580c"; }
  else if (folderName === "Template-026") { primaryColor = "#1e3a8a"; accentColor = "#d97706"; }
  else if (folderName === "Template-027") { primaryColor = "#09090b"; accentColor = "#ca8a04"; }
  else if (folderName === "Template-028") { primaryColor = "#8b5cf6"; accentColor = "#06b6d4"; }
  else if (folderName === "Template-029") { primaryColor = "#0f766e"; accentColor = "#0d9488"; }
  else if (folderName === "Template-030") { primaryColor = "#0284c7"; accentColor = "#6366f1"; }

  if (isDarkSidebar) {
    // Left Sidebar
    ctx.fillStyle = primaryColor;
    ctx.fillRect(0, 0, 130, 520);

    // Sidebar Content
    ctx.fillStyle = "#ffffff";
    ctx.font = "bold 14px sans-serif";
    ctx.fillText("ALEX R.", 15, 35);
    ctx.font = "10px sans-serif";
    ctx.fillStyle = accentColor;
    ctx.fillText("SOFTWARE FRESHER", 15, 50);

    // Sidebar divider
    ctx.strokeStyle = "#334155";
    ctx.beginPath(); ctx.moveTo(15, 60); ctx.lineTo(115, 60); ctx.stroke();

    // Contact
    ctx.fillStyle = "#94a3b8"; ctx.font = "9px sans-serif";
    ctx.fillText("alex@univ.edu", 15, 80);
    ctx.fillText("+1 555-0192", 15, 95);
    ctx.fillText("github/alex-dev", 15, 110);

    // Sidebar Skills
    ctx.fillStyle = accentColor; ctx.font = "bold 10px sans-serif";
    ctx.fillText("CORE SKILLS", 15, 140);
    ctx.fillStyle = "#ffffff"; ctx.font = "9px sans-serif";
    ["Java / Python", "React.js / SQL", "Data Structures", "Git / Linux"].forEach((s, idx) => {
      ctx.fillText(`• ${s}`, 15, 160 + (idx * 16));
    });

    // Main Right Content
    ctx.fillStyle = primaryColor; ctx.font = "bold 12px sans-serif";
    ctx.fillText("CAREER OBJECTIVE", 150, 35);
    ctx.fillStyle = "#475569"; ctx.font = "10px sans-serif";
    ctx.fillText("Enthusiastic CS graduate with strong problem-solving skills.", 150, 52);

    ctx.fillStyle = primaryColor; ctx.font = "bold 12px sans-serif";
    ctx.fillText("KEY ACADEMIC PROJECTS", 150, 85);
    ctx.strokeStyle = accentColor; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.moveTo(150, 90); ctx.lineTo(380, 90); ctx.stroke();

    // Project 1
    ctx.fillStyle = "#0f172a"; ctx.font = "bold 11px sans-serif";
    ctx.fillText("Smart Campus Portal (Full Stack)", 150, 110);
    ctx.fillStyle = accentColor; ctx.font = "bold 9px sans-serif";
    ctx.fillText("Tech: React, Node.js, PostgreSQL", 150, 124);
    ctx.fillStyle = "#334155"; ctx.font = "9px sans-serif";
    ctx.fillText("Built student management portal serving 2000+ users.", 150, 138);

    // Project 2
    ctx.fillStyle = "#0f172a"; ctx.font = "bold 11px sans-serif";
    ctx.fillText("AI Resume Optimizer & ATS Matcher", 150, 165);
    ctx.fillStyle = accentColor; ctx.font = "bold 9px sans-serif";
    ctx.fillText("Tech: Python, NLP, Express API", 150, 179);
    ctx.fillStyle = "#334155"; ctx.font = "9px sans-serif";
    ctx.fillText("Implemented TF-IDF keyword extraction algorithm.", 150, 193);

    // Education
    ctx.fillStyle = primaryColor; ctx.font = "bold 12px sans-serif";
    ctx.fillText("EDUCATION", 150, 230);
    ctx.strokeStyle = accentColor; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.moveTo(150, 235); ctx.lineTo(380, 235); ctx.stroke();

    ctx.fillStyle = "#0f172a"; ctx.font = "bold 11px sans-serif";
    ctx.fillText("B.Tech in Computer Science Engineering", 150, 255);
    ctx.fillStyle = "#64748b"; ctx.font = "9px sans-serif";
    ctx.fillText("State Tech University (2022 - 2026) | GPA: 3.9/4.0", 150, 270);
  } else {
    // Top Banner
    ctx.fillStyle = primaryColor;
    ...
    ctx.fillText("ALEX RIVERS", 20, 32);
    ctx.fillStyle = accentColor; ctx.font = "bold 11px sans-serif";
    ctx.fillText(tplData.name.toUpperCase(), 20, 52);

    // Contact Bar
    ctx.fillStyle = "#f8fafc"; ctx.fillRect(0, 70, 400, 24);
    ctx.fillStyle = "#475569"; ctx.font = "9px sans-serif";
    ctx.fillText("alex@univ.edu  |  +1 555-0192  |  github.com/alex-dev  |  linkedin.com/in/alex", 15, 86);

    // Section 1: Objective
    ctx.fillStyle = primaryColor; ctx.font = "bold 11px sans-serif";
    ctx.fillText("CAREER OBJECTIVE", 20, 115);
    ctx.strokeStyle = accentColor; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.moveTo(20, 120); ctx.lineTo(380, 120); ctx.stroke();

    ctx.fillStyle = "#334155"; ctx.font = "9.5px sans-serif";
    ctx.fillText("Passionate Computer Science graduate seeking entry-level software engineer role.", 20, 136);

    // Section 2: Projects
    ctx.fillStyle = primaryColor; ctx.font = "bold 11px sans-serif";
    ctx.fillText("FEATURED ACADEMIC PROJECTS", 20, 168);
    ctx.strokeStyle = accentColor; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.moveTo(20, 173); ctx.lineTo(380, 173); ctx.stroke();

    // Project 1 Box
    ctx.fillStyle = "#f8fafc"; ctx.fillRect(20, 182, 360, 58);
    ctx.strokeStyle = "#e2e8f0"; ctx.strokeRect(20, 182, 360, 58);
    ctx.fillStyle = primaryColor; ctx.font = "bold 11px sans-serif";
    ctx.fillText("Smart Campus Management System (Full Stack)", 30, 198);
    ctx.fillStyle = accentColor; ctx.font = "bold 9px sans-serif";
    ctx.fillText("Stack: React.js, Node.js, Express, PostgreSQL", 30, 212);
    ctx.fillStyle = "#475569"; ctx.font = "9px sans-serif";
    ctx.fillText("Designed student grading and attendance module serving 2000+ active users.", 30, 226);

    // Project 2 Box
    ctx.fillStyle = "#f8fafc"; ctx.fillRect(20, 248, 360, 58);
    ctx.strokeStyle = "#e2e8f0"; ctx.strokeRect(20, 248, 360, 58);
    ctx.fillStyle = primaryColor; ctx.font = "bold 11px sans-serif";
    ctx.fillText("AI Resume Parser & Skill Matching Engine", 30, 264);
    ctx.fillStyle = accentColor; ctx.font = "bold 9px sans-serif";
    ctx.fillText("Stack: Python, NLP, Scikit-learn, REST API", 30, 278);
    ctx.fillStyle = "#475569"; ctx.font = "9px sans-serif";
    ctx.fillText("Achieved 94% matching accuracy for fresher technical resumes.", 30, 292);

    // Section 3: Education & Skills (2 columns)
    // Left: Education
    ctx.fillStyle = primaryColor; ctx.font = "bold 11px sans-serif";
    ctx.fillText("EDUCATION", 20, 332);
    ctx.strokeStyle = accentColor; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.moveTo(20, 337); ctx.lineTo(190, 337); ctx.stroke();

    ctx.fillStyle = "#0f172a"; ctx.font = "bold 10px sans-serif";
    ctx.fillText("B.Tech Computer Science", 20, 354);
    ctx.fillStyle = "#64748b"; ctx.font = "9px sans-serif";
    ctx.fillText("State Tech University (2022-2026)", 20, 368);
    ctx.fillStyle = accentColor; ctx.font = "bold 9px sans-serif";
    ctx.fillText("GPA / Result: 3.9 / 4.0", 20, 382);

    // Right: Skills
    ctx.fillStyle = primaryColor; ctx.font = "bold 11px sans-serif";
    ctx.fillText("TECHNICAL SKILLS", 210, 332);
    ctx.strokeStyle = accentColor; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.moveTo(210, 337); ctx.lineTo(380, 337); ctx.stroke();

    const skillsList = ["JavaScript / ES6+", "Python / C++", "React.js / Redux", "SQL / PostgreSQL", "Git / GitHub / CI/CD"];
    skillsList.forEach((sk, idx) => {
      ctx.fillStyle = "#f1f5f9"; ctx.fillRect(210, 345 + (idx * 20), 165, 16);
      ctx.fillStyle = primaryColor; ctx.font = "9px sans-serif";
      ctx.fillText(sk, 216, 357 + (idx * 20));
    });
  }

  // Footer Tag
  ctx.fillStyle = primaryColor;
  ctx.fillRect(0, 495, 400, 25);
  ctx.fillStyle = "#ffffff";
  ctx.font = "bold 10px sans-serif";
  ctx.fillText(`PREMIUM FRESHER TEMPLATE  |  ${folderName}`, 80, 512);

  return canvas.toBuffer("image/png");
}

async function main() {
  console.log("Adding 15 Premium Fresher Templates (Template-016 to Template-030)...");

  // 1. Generate HTML, CSS & Preview PNGs
  for (const [folderName, data] of Object.entries(newTemplates)) {
    ...
    const pngBuffer = generatePreviewPNG(folderName, data);
    fs.writeFileSync(previewPath, pngBuffer);

    console.log(`[Success] Written HTML, CSS & preview.png for ${folderName}: ${data.name}`);
  }

  // 2. Database Upsert for all 30 templates
  const client = await pool.connect();
  try {
    console.log("Upserting templates into PostgreSQL database...");
    let displayOrder = 16;
    for (const [folderName, data] of Object.entries(newTemplates)) {
      ...
      await client.query(`
        ...
        VALUES ($1, $2, $3, 'preview.png', $4, true, $5)
      ...
      `, [templateId, data.name, folderName, data.is_premium, displayOrder]);

      displayOrder++;
    }

    // Reset primary key sequence
    await client.query("SELECT setval('resume_templates_template_id_seq', (SELECT MAX(template_id) FROM resume_templates))");
    ...
    console.log(`[DB Success] Total active templates in DB: ${res.rows.length}`);
    console.log("Sample DB Rows:", res.rows.slice(-5));
  } catch (err) {
    console.error("Database error during template seed:", err);
  } finally {
    client.release();
  }

  console.log("Finished adding all 15 freshers templates!");
}
